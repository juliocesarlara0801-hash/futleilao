import type { Bid, GameMode, Player, Position, Room, Team } from '../types.js';
import { FORMATION_REQUIREMENTS } from '../types.js';
...
/**
 * Gera o pool de jogadores para o leilão: por posição, poolCount = requiredPerTeam * (N + 2).
 * Isso reproduz exatamente as tabelas do modo futsal (N+2 / 2N+4 / N+2 / N+2) e generaliza
 * proporcionalmente para o modo completo.
 */
...
/** Regra de reserva de saldo: lance máximo = saldo - (vagas restantes - 1), mínimo R$1 por vaga futura. */
...
/**
 * Resolve os lances fechados de uma rodada. Empate no valor é decidido por quem tem
 * MENOS jogadores naquela posição (prioridade a quem mais precisa); se ainda empatar, sorteio.
 */
...
/**
 * Distribuição automática: quando o nº de jogadores restantes de uma posição no pool
 * é igual ao nº de times que ainda precisam dessa posição, distribui por sorteio a R$1 cada.
 */
export function computeAutoAssignments(
  ...
  const teamsNeeding = teams.filter(
    (t) => !t.isSpectator && vacanciesForPosition(t, requirements, position) > 0
  );
...
export function allTeamsComplete(room: Room, requirements: FormationRequirements): boolean {
  return room.teams
    .filter((t) => !t.isSpectator)
    .every((t) => remainingVacancies(t, requirements) <= 0);
}
...
/**
 * Marca 1-2 jogadores existentes de rating alto como coringas e injeta 1 jogador
 * fictício (meme) com rating baixo em posição aleatória do pool, para dar emoção.
 */
