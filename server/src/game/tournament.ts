...
import { shuffle } from './auction.js';
...
/** Mata-mata simples: 4 (semi+final) ou 8 (quartas+semi+final) times, chaveamento aleatório. */
export function buildKnockoutFirstRound(teams: Team[]): { matches: MatchResult[]; bracket: BracketRound[] } {
  const playable = shuffle(teams.filter((t) => !t.isSpectator));
...
export function buildNextKnockoutRound(
  previousRoundMatches: MatchResult[],
  teams: Team[]
): { matches: MatchResult[]; roundName: string } | null {
  ...
  for (let i = 0; i < winners.length; i += 2) {
    scheduled.push({ id: randomUUID(), round: roundName, homeTeamId: winners[i], awayTeamId: winners[i + 1] });
  }
...
/** Copa do Mundo (16 times): 4 grupos de 4 (pontos corridos) + oitavas + quartas + semi + final. */
...
function aggregatePlayers(matches: MatchResult[], teams: Team[]): Map<string, PlayerAggregate> {
  ...
  const teamOf = (teamId: string, playerName: string) => {
    ...
    if (!agg.has(key)) {
      ...
      agg.set(key, { name: playerName, teamId, teamName: team?.teamName ?? '', goals: 0, assists: 0, saves: 0, matchScore: 0 });
    }
    return agg.get(key)!;
  };
...
export function computeAwards(
  ...
  if (finalStandings[0]) {
    awards.push({ type: 'champion', winner: finalStandings[0].teamName, teamName: finalStandings[0].teamName });
  }
  if (finalStandings[1]) {
    awards.push({ type: 'runnerUp', winner: finalStandings[1].teamName, teamName: finalStandings[1].teamName });
  }
  if (finalStandings[2]) {
    awards.push({ type: 'third', winner: finalStandings[2].teamName, teamName: finalStandings[2].teamName });
  }
...
export const ROUND_ORDER: Record<TournamentFormat, string[]> = {
  league: ['Fase única'],
  knockout: ['Quartas de final', 'Semifinal', 'Final'],
  worldcup: ['Grupos', 'Oitavas de final', 'Quartas de final', 'Semifinal', 'Final'],
};
